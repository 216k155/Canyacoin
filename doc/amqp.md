...
during the *configure* step of building canyad:
...
Launch canyad like this:

    $ canyad -amqppubhashtx=amqp://127.0.0.1:5672
...
These options can also be provided in canya.conf.
...
From the perspective of canyad, the local end of an AMQP link is write-only.

No information is broadcast that wasn't already received from the public
P2P network.

No authentication or authorization is done on peers that canyad connects
...
At present, canyad does not try to resend a notification if there was
...
Currently, canyad appends an up-counting sequence number to each notification
