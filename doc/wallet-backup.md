...
For all methods, you will need to include an export directory setting in your config file (`canya.conf` located in the data directory which is `~/.canya/` unless it's been overridden with `datadir=` setting):
...
Note that canyad will need to be stopped and restarted for edits in the config file to take effect. 

### Using `backupwallet`

To create a backup of your wallet, use:

`canya-cli backupwallet <nameofbackup>`.
...
`canya-cli z_exportwallet <nameofbackup>`
...
`canya-cli z_importwallet </path/to/exportdir/nameofbackup>`
...
`canya-cli z_exportkey <z-address>`
...
`canya-cli dumpprivkey <t-address>`

This will return the private key and will not create a new file.

To import a private key for a shielded address, use:

`canya-cli z_importkey <z-priv-key>`
