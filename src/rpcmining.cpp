...
UniValue getgenerate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or canya.conf setting gen)\n"
...
#else
        throw JSONRPCError(RPC_METHOD_NOT_FOUND, "canyad compiled without wallet and -mineraddress not set");
#endif
    }
...
UniValue getblocksubsidy(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"miner\" : x.xxx           (numeric) The mining reward amount in CAN.\n"
            "  \"community\" : x.xxx        (numeric) The community fund amount in CAN.\n"
            "}\n"
            "\nExamples:\n"
