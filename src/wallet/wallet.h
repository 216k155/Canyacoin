...
{
public:
    libcanya::PaymentAddress address;
    ...
    CNoteData(libcanya::PaymentAddress a) :
            address {a}, nullifier(), witnessHeight {-1} { }
    CNoteData(libcanya::PaymentAddress a, uint256 n) :
...
{
    JSOutPoint jsop;
    libcanya::NotePlaintext plaintext;
};
...
public:
    ...
    std::map<libcanya::PaymentAddress, CKeyMetadata> mapZKeyMetadata;
    ...
    bool AddZKey(const libcanya::SpendingKey &key);
    //! Adds spending key to the store, without saving it to disk (used by LoadWallet)
    bool LoadZKey(const libcanya::SpendingKey &key);
    //! Load spending key metadata (used by LoadWallet)
    bool LoadZKeyMetadata(const libcanya::PaymentAddress &addr, const CKeyMetadata &meta);
    //! Adds an encrypted spending key to the store, without saving it to disk (used by LoadWallet)
    bool LoadCryptedZKey(const libcanya::PaymentAddress &addr, const libcanya::ViewingKey &vk, const std::vector<unsigned char> &vchCryptedSecret);
    //! Adds an encrypted spending key to the store, and saves it to disk (virtual method, declared in crypter.h)
    bool AddCryptedSpendingKey(const libcanya::PaymentAddress &address, const libcanya::ViewingKey &vk, const std::vector<unsigned char> &vchCryptedSecret);
    ...
    boost::optional<uint256> GetNoteNullifier(
        ...
        const libcanya::PaymentAddress& address,
