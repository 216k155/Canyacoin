...
bool CWallet::AddZKey(const libcanya::SpendingKey &key)
{
    AssertLockHeld(cs_wallet); // mapZKeyMetadata
...
bool CWallet::AddCryptedSpendingKey(const libcanya::PaymentAddress &address,
                                    const libcanya::ViewingKey &vk,
...
bool CWallet::LoadCryptedZKey(const libcanya::PaymentAddress &addr, const libcanya::ViewingKey &vk, const std::vector<unsigned char> &vchCryptedSecret)
{
    return CCryptoKeyStore::AddCryptedSpendingKey(addr, vk, vchCryptedSecret);
}

bool CWallet::LoadZKey(const libcanya::SpendingKey &key)
{
            ...
            for (mapNoteData_t::value_type& item : wtxItem.second.mapNoteData) {
                if (!item.second.nullifier) {
                    ...
                    auto hSig = wtxItem.second.vjoinsplit[i].h_sig(
                        *pcanyaParams, wtxItem.second.joinSplitPubKey);
                                                   ...
                                                   uint8_t n) const
{
    ...
    auto note_pt = libcanya::NotePlaintext::decrypt(
        dec,
...
mapNoteData_t CWallet::FindMyNotes(const CTransaction& tx) const
{
    ...
    for (size_t i = 0; i < tx.vjoinsplit.size(); i++) {
        auto hSig = tx.vjoinsplit[i].h_sig(*pcanyaParams, tx.joinSplitPubKey);
...
void CWallet::GetFilteredNotes(std::vector<CNotePlaintextEntry> & outEntries, std::string address, int minDepth, bool ignoreSpent)
{
    ...
    libcanya::PaymentAddress filterPaymentAddress;
    if (address.length() > 0) {
    ...
    for (auto & p : mapWallet) {
        ...
        for (auto & pair : wtx.mapNoteData) {
            ...
            auto hSig = wtx.vjoinsplit[i].h_sig(*pcanyaParams, wtx.joinSplitPubKey);
            try {
