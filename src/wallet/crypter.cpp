...
static bool DecryptSpendingKey(const CKeyingMaterial& vMasterKey,
                               ...
                               libcanya::SpendingKey& sk)
{
    CKeyingMaterial vchSecret;
    if(!DecryptSecret(vMasterKey, vchCryptedSecret, address.GetHash(), vchSecret))
        return false;

    if (vchSecret.size() != libcanya::SerializedSpendingKeySize)
        return false;
...
{
    {
        ...
        {
            const libcanya::PaymentAddress &address = (*skmi).first;
...
bool CCryptoKeyStore::AddSpendingKey(const libcanya::SpendingKey &sk)
{
    {
...
bool CCryptoKeyStore::AddCryptedSpendingKey(const libcanya::PaymentAddress &address,
...
bool CCryptoKeyStore::GetSpendingKey(const libcanya::PaymentAddress &address, libcanya::SpendingKey &skOut) const
{
    {
        LOCK(cs_SpendingKeyStore);
        if (!IsCrypted())
...
{
    {
        ...
        {
            const libcanya::SpendingKey &sk = mSpendingKey.second;
            CSecureDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
            ss << sk;
            CKeyingMaterial vchSecret(ss.begin(), ss.end());
            libcanya::PaymentAddress address = sk.address();
            std::vector<unsigned char> vchCryptedSecret;
