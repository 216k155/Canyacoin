...
CWalletTx GetValidReceive(const libcanya::SpendingKey& sk, CAmount value, bool randomInputs) {
    return GetValidReceive(*params, sk, value, randomInputs);
}

libcanya::Note GetNote(const libcanya::SpendingKey& sk,
                       const CTransaction& tx, size_t js, size_t n) {
    return GetNote(*params, sk, tx, js, n);
}

CWalletTx GetValidSpend(const libcanya::SpendingKey& sk,
...
TEST(wallet_tests, FindMyNotes) {
    ...
    auto sk = libcanya::SpendingKey::random();
    auto sk2 = libcanya::SpendingKey::random();
