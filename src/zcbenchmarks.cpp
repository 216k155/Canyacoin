...
#include "canya/Zcash.h"
#include "canya/IncrementalMerkleTree.hpp"
...
double benchmark_create_joinsplit()
{
    ...
    assert(jsdesc.Verify(*pcanyaParams, verifier, pubKeyHash));
    return ret;
}
...
double benchmark_verify_joinsplit(const JSDescription &joinsplit)
{
    ...
    uint256 pubKeyHash;
    auto verifier = libcanya::ProofVerifier::Strict();
    joinsplit.Verify(*pcanyaParams, verifier, pubKeyHash);
    return timer_stop(tv_start);
}
...
double benchmark_try_decrypt_notes(size_t nAddrs)
{
    ...
    for (int i = 0; i < nAddrs; i++) {
        auto sk = libcanya::SpendingKey::random();
        wallet.AddSpendingKey(sk);
    }

    auto sk = libcanya::SpendingKey::random();
    auto tx = GetValidReceive(*pcanyaParams, sk, 10, true);
...
double benchmark_increment_note_witnesses(size_t nTxs)
{
    ...
    for (int i = 0; i < nTxs; i++) {
        auto wtx = GetValidReceive(*pcanyaParams, sk, 10, true);
        auto note = GetNote(*pcanyaParams, sk, wtx, 0, 1);
