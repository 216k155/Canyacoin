...
public:
    bool Set(const libcanya::PaymentAddress& addr);
    CZCPaymentAddress() {}

    CZCPaymentAddress(const std::string& strAddress) { SetString(strAddress.c_str(), 2); }
    CZCPaymentAddress(const libcanya::PaymentAddress& addr) { Set(addr); }

    libcanya::PaymentAddress Get() const;
};

class CZCSpendingKey : public CBase58Data {
public:
    bool Set(const libcanya::SpendingKey& addr);
    CZCSpendingKey() {}

    CZCSpendingKey(const std::string& strAddress) { SetString(strAddress.c_str(), 2); }
    CZCSpendingKey(const libcanya::SpendingKey& addr) { Set(addr); }

    libcanya::SpendingKey Get() const;
};
