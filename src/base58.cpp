...
bool CZCPaymentAddress::Set(const libcanya::PaymentAddress& addr)
{
    CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
    ss << addr;
    std::vector<unsigned char> addrSerialized(ss.begin(), ss.end());
    assert(addrSerialized.size() == libcanya::SerializedPaymentAddressSize);
    SetData(Params().Base58Prefix(CChainParams::ZCPAYMENT_ADDRRESS), &addrSerialized[0], libcanya::SerializedPaymentAddressSize);
    return true;
}

libcanya::PaymentAddress CZCPaymentAddress::Get() const
{
    if (vchData.size() != libcanya::SerializedPaymentAddressSize) {
    ...
    libcanya::PaymentAddress ret;
    ss >> ret;
    return ret;
}

bool CZCSpendingKey::Set(const libcanya::SpendingKey& addr)
{
    CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
    ss << addr;
    std::vector<unsigned char> addrSerialized(ss.begin(), ss.end());
    assert(addrSerialized.size() == libcanya::SerializedSpendingKeySize);
    SetData(Params().Base58Prefix(CChainParams::ZCSPENDING_KEY), &addrSerialized[0], libcanya::SerializedSpendingKeySize);
    return true;
}

libcanya::SpendingKey CZCSpendingKey::Get() const
{
    if (vchData.size() != libcanya::SerializedSpendingKeySize) {
        throw std::runtime_error(
