...
#if defined(NDEBUG)
# error "Can cannot be compiled without assertions."
#endif
                      ...
                      libcanya::ProofVerifier& verifier)
{
    ...
    BOOST_FOREACH(const JSDescription &joinsplit, tx.vjoinsplit) {
        if (!joinsplit.Verify(*pcanyaParams, verifier, tx.joinSplitPubKey)) {
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, const CChain& chain, bool fJustCheck)
{
    ...
    auto verifier = libcanya::ProofVerifier::Strict();
    auto disabledVerifier = libcanya::ProofVerifier::Disabled();
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, bool fRequested, CDiskBlockPos* dbp)
{
    ...
    auto verifier = libcanya::ProofVerifier::Disabled();
