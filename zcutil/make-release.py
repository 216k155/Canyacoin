...
def build():
    ...
    sh_progress([
        ...
        'config.status: creating libcanyaconsensus.pc',
...
def patch_gitian_linux_yml(release, releaseprev):
    ...
    with PathPatcher(path) as (inf, outf):
        ...
        assert secondline == 'name: "canya-{}"\n'.format(
            releaseprev.novtext
        ), repr(secondline)

        outf.write('name: "canya-{}"\n'.format(release.novtext))
        outf.write(inf.read())
...
def initialize_logging():
    logname = './canya-make-release.log'
    ...
    logging.info('canya make-release.py debug log: %r', logname)
