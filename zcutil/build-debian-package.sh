...
PACKAGE_VERSION=$($SRC_PATH/src/canyad --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/canyad $DEB_BIN
cp $SRC_PATH/src/canya-cli $DEB_BIN
cp $SRC_PATH/zcutil/fetch-params.sh $DEB_BIN/canya-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/canyad.1 $DEB_MAN
cp $SRC_DOC/man/canya-cli.1 $DEB_MAN
cp $SRC_DOC/man/canya-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/bitcoind.bash-completion $DEB_CMP/canyad
cp $SRC_PATH/contrib/bitcoin-cli.bash-completion $DEB_CMP/canya-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/canyad.1
gzip --best -n $DEB_MAN/canya-cli.1
gzip --best -n $DEB_MAN/canya-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/canyad $DEB_BIN/canya-cli
